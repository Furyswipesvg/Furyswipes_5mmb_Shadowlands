...
proc pop { raid } {
	...
	set curraid $raid
	if { ! [ dict exists $allraids ${raid}1] } { puts "Error, no such raid $raid" ; return }
	...
	foreach mywin [dict get $allraids ${curraid}1] {
		...
		if {  [twapi::find_windows -single  -messageonlywindow false -popup false -toplevel true -visible true -text $winname ] == "" } {
			incr numwins
	        } else { lappend existing_wins $winname } 
	}
...
proc 5mmb_monitor { args } {
	if { [twapi::GetKeyState 0x91] == 0 } { return }
	...
	foreach arg $args {
		if { $arg=="-anypressed" } { set ANYPRESSED true ; continue }
		if { $arg=="-keyup" } { set KEYUP true ; continue }
		if { $arg=="-movekey" } { set MOVEKEY true ; continue }
		if { $keylist=="" } { set keylist $arg ; continue }
		if { $script=="" } { set script $arg }
	}
	5mmb_update_keystate $keylist
	foreach key $keylist {
		if {$key == "TRIGGER" } { set add_key_as_script_arg true ; continue }
		if { [regexp "^!" $key] } { 
			set key [regsub "!" $key ""]
...
proc wow_is_focused { } {
	...
	if { $mywin!="" } {
	   if { [lsearch $mytoons [twapi::get_window_text $mywin]] > -1 } { return true } 
   	}
	return false
}
proc getwin { name } {
	...
	set existing_wins ""
	foreach mypid $wow_pids { 
	   if { [set mywin [twapi::find_windows -single -messageonlywindow false -popup false -toplevel true -visible true -pids $mypid -text $name]] != "" } { return $mywin }
   	}
}
...
while { ($game == "shadow" || $game == "classic")} {
	...
	if { [twapi::GetKeyState 0x91] && [expr $idletimer + $returndelay]  < [clock milliseconds] && [wow_is_focused] } { resettimer ; reset_rotations } 
}	
